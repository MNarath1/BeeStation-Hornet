...
/datum/controller/subsystem/lighting/proc/get_sources_in_viewer_range(atom/movable/lighting_mask_holder/viewer)
	. = list()
	if(viewer.grid_z)
		for(var/x in GET_CLAMPED_DELTA(viewer.x, viewer.viewer_width, world.maxx))
			for(var/y in GET_CLAMPED_DELTA(viewer.y, viewer.viewer_height, world.maxy))
				//TODO Somehow optimise this
				. |= SSlighting.light_source_grid[viewer.z][x][y][LIGHT_EXPOSED]
